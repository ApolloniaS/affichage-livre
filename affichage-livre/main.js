...
// créa et récup de divs avec classe bd
var nbBD = bds.length;
for (var i = 0; i < nbBD; i++) {
  main.innerHTML += '<div class="bd"></div>';
}
var divBD = document.querySelectorAll('.bd');
var button = "<input class='btn' type='button' value='Emprunter cette BD !'>";

// affichage
for (var j = 0; j < nbBD; j++) {
  divBD[j].innerHTML += `<br><img src="images\\static\\${bds[j].image}"><br>`;
  divBD[j].innerHTML += `Id: ${bds[j].id}<br>`;
  divBD[j].innerHTML += `<span id="title">Titre: ${bds[j].titre}</span> <br>`;
  divBD[j].innerHTML += `Editeur: ${bds[j].editeur} <br>`;
  divBD[j].innerHTML += `Collection: ${bds[j].collection}<br>`;
  divBD[j].innerHTML += `Serie: ${bds[j].serie}<br>`;
  divBD[j].innerHTML += `Type: ${bds[j].type}<br>`;
  divBD[j].innerHTML += `Etat: ${bds[j].etat}<br>`;
  divBD[j].innerHTML += `Isbn: ${bds[j].isbn}<br>`;
  divBD[j].innerHTML += `Annee de parution: ${bds[j].annee_de_parution}<br>`;
  divBD[j].innerHTML += `N° tome: ${bds[j].no_tome}<br>`;
  divBD[j].innerHTML += `Prix: ${bds[j].prix}€<br>`;
  divBD[j].innerHTML += `Themes: ${bds[j].themes}<br>`;
  divBD[j].innerHTML += `Auteurs: ${bds[j].auteurs}<br>`;
  divBD[j].innerHTML += `Like: ${bds[j].like}<br>`;
  divBD[j].innerHTML += `Emprunt: ${bds[j].emprunt}<br>`;
  divBD[j].innerHTML += `Résume: ${bds[j].resume.slice(0, 30)}<span id="readMore">... (lire la suite)</span><br>`;
  divBD[j].innerHTML += `<span class="fullSummary">${bds[j].resume.slice(30)}</span><br>`;
  divBD[j].innerHTML += button;
}

// ajout du panier et récupération des éléments nécéssaires pour les events
main.innerHTML += '<div id="cart">VOTRE PANIER</div>';
var cart = document.querySelector('#cart');
var readMore = document.querySelectorAll('#readMore');
var buttons = document.querySelectorAll('.btn');

// afficher le résumé en entier
for (var el of readMore) {
  el.addEventListener('click', function (e) {
    var fullSumarry = e.target.parentNode.children[21].innerHTML;
    e.target.innerHTML = fullSumarry;
  });
}
...
buttons.forEach(function (btn) {
  btn.addEventListener('click', function (e) {
    var title = e.target.parentNode.children[4].innerHTML;
    var toBuy = title.slice(7);
    cart.innerHTML += `<br>${toBuy}`;
  });
});
